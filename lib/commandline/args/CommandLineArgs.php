...
class CommandLineArgs
{
	...
	function process()
	{
		...
		$this->checkEntity();
		$this->checkActions();
		ProgramConfig::sortActionsByPriority();
	}
	...
	function processEntityType()
	{
		...
		while(!$this->EOArgs())
		{
			if($this->currentArgTypeIsEntityType())
			{
				...
				ProgramConfig::$entityType=$entityType;
	...
	function processCurrentArg()
	{
		if($this->currentArgTypeIsEntityType())
		{
				...
				$this->argsError("Unexpected entity type '$entityType'. ".
					...
					ProgramConfig::$entityType."'",false, 
	...
	function processAction()
	{
		...
		$entityType=ProgramConfig::$entityType;

		$action=$this->actionFromCurrentArg();
		//check if it is a duplicate
		if(ProgramConfig::actionExists($action))
		...
		$firstIncompatibleAction=$this->commandActionDefinitions->firstIncompatibleAction(
						$entityType,ProgramConfig::actionArray(),$action);
		...
		$priority=$this->commandActionDefinitions->actionPriority($entityType,$action);
		//Ok. Add it.
		ProgramConfig::addAction(new CommandAction($action,$actionArg,$priority));
	}

	function checkEntity()
	{
		$entityType=ProgramConfig::$entityType;
		$entityName=ProgramConfig::$entityName;
	...
	function processEntity()
	{
		...
		if(ProgramConfig::$entityName!=null)
			$this->argsError("Unexpected entity '$entityName'. ".
				"Already processing entity ".ProgramConfig::$entityName,
				true,ERRNUM_UNEXPECTED_ENTITY_TYPE);
		ProgramConfig::$entityName=$entityName;
	}
